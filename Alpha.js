			...
			const getGroup = async function(totalchat){
				...
				if (authorname != undefined) { } else { authorname = groupName }	
					function addMetadata(packname, author) {	
						if (!packname) packname = 'ExBotZ'; if (!author) author = 'bot';author = author.replace(/[^a-zA-Z0-9]/g, '');	
				...
				download(url, namea, async function () {
					...
					const fakelokasi = {key: {fromMe: false, participant: `${numbernye}@s.whatsapp.net`, ...(from ? {remoteJid: "status@broadcast" } : {}) }, message: {locationMessage: {degreesLatitude: -7.0389318, degreesLongitude: 113.8969749, name: 'Rusia', address: '@ekuzikaa_18', jpegThumbnail: thumbnail}}}
...
break
						...
						case 'nulis': case 'tulis':
									...
									res = await axios.get(`https://api.zeks.me/api/nulis?text=${nulis}&apikey=Api-Zeks-Xyz`)
